...
new = '''    def _signer(self, horodatage: int, methode: str, chemin: str,\n                corps_json: str = "") -> str:\n        """Signe exactement le JSON qui sera transmis a Bitvavo."""\n        methode = str(methode).upper()\n        message = f"{int(horodatage)}{methode}/v2{chemin}" + (corps_json or "")\n        return hmac.new(\n            self.config.api_secret.encode("utf-8"),\n            message.encode("utf-8"),\n            hashlib.sha256,\n        ).hexdigest()\n'''
...
new2 = '''        entetes = {"User-Agent": "gold-bot/1.0", "Accept": "application/json"}\n        corps_json = (json.dumps(corps, separators=(",", ":"), ensure_ascii=False)\n                      if corps is not None else "")\n        donnees = corps_json.encode("utf-8") if corps_json else None\n        if corps_json:\n            entetes["Content-Type"] = "application/json"\n\n        if signe:\n            if not (self.config.api_key and self.config.api_secret):\n                raise BrokerError("BITVAVO_API_KEY et BITVAVO_API_SECRET absents")\n            horodatage = int(time.time() * 1000)\n            methode = str(methode).upper()\n            entetes.update({\n                "bitvavo-access-key": self.config.api_key,\n                "bitvavo-access-signature": self._signer(horodatage, methode, requete, corps_json),\n                "bitvavo-access-timestamp": str(horodatage),\n                "bitvavo-access-window": str(self.config.window),\n            })\n'''

if old2 in text:
    text = text.replace(old2, new2, 1)

if old not in text and old2 not in text:
    print("Les blocs anciens sont deja corriges ou ont change; aucune modification necessaire")
else:

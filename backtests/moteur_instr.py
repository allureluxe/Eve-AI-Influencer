    ...
    for t in jours:
        ...
        for paire in list(ouvertes):
            ...
            if a > 0 and pos.risque_initial > 0:
                ...
                # CLIQUET, ARME SUR UNE PROGRESSION EN ATR.
                #
                # Le declencheur ne peut pas etre le R : `risque_initial`
                # grossit a chaque etage, donc le R d'une position
                # renforcee ne repasse jamais au-dessus du seuil et le
                # suiveur ne se rearme plus. On mesure donc la
                # progression en ATR, qui ne depend d'aucun denominateur
                # mobile. `trail_depart_r` reste exprime en R pour ne pas
                # changer le reglage : 1,1 R = 1,1 x stop_atr ATR.
                #
                # C'est la meme correction que dans le moteur reel. Sans
                # elle, toute mesure du pyramidage compare des pommes a
                # des poires : elle empile des etages sur un stop fige.
                progres_atr = (pos.plus_haut - pos.entree) / a
                if (gain_r >= r.trail_depart_r
                        or progres_atr >= r.trail_depart_r * r.stop_atr
                        or pos.stop > pos.entree
                        or pos.etages > 1):
                    pos.trail_arme = True
                if pos.trail_arme:

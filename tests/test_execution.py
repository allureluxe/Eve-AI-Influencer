...
from gold_bot.brokers.moonx import MoonXBroker, MoonXConfig
...
class TestMoonX(unittest.TestCase):
    def test_mode_non_configure_detecte(self):
        broker = MoonXBroker(MoonXConfig())
        self.assertEqual(broker.mode, "non configure")
        self.assertFalse(broker.connect())

    def test_mode_simulation(self):
        broker = MoonXBroker(MoonXConfig(dry_run=True))
        self.assertTrue(broker.connect())
        self.assertIn("dry-run", broker.mode)

    def test_mode_pont_ecrit_les_ordres(self):
        fd, path = tempfile.mkstemp(suffix=".jsonl")
        os.close(fd)
        os.unlink(path)
        try:
            universe = Universe()
            gold = universe.get("XAUUSD")
            broker = MoonXBroker(MoonXConfig(bridge_file=path))
            broker.register_instrument(gold)
            self.assertTrue(broker.connect())
            broker.open_position(gold, Side.BUY, 0.1, 2645.0, 2660.0)
            with open(path, encoding="utf-8") as fh:
                lignes = fh.readlines()
            self.assertEqual(len(lignes), 1)
            self.assertIn('"stopLoss": 2645.0', lignes[0])
        finally:
            os.path.exists(path) and os.unlink(path)

    def test_ordre_sans_stop_refuse(self):
        universe = Universe()
        broker = MoonXBroker(MoonXConfig(dry_run=True))
        broker.connect()
        with self.assertRaises(BrokerError):
            broker.open_position(universe.get("XAUUSD"), Side.BUY, 0.1, 0.0, 2660.0)
